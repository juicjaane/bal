...
def home():
    ...
    if db:
        try:
            ...
            logger.error(f"Corporate stats error: {e}")
    
    trees_funded = int(total_donated / 10)  # ~₹10 per tree estimate
    carbon_offset = round(total_donated * 0.0067, 1)  # rough estimate
    ...
    if db:
        try:
            ...
            for d in donations_list:
                ngo_name = d.get('ngo_name', 'Unknown')
                ngo_breakdown[ngo_name] = ngo_breakdown.get(ngo_name, 0) + d.get('amount', 0)
        except Exception as e:
            logger.error(f"Impact fetch error: {e}")
    
    trees_funded = int(total / 10)
    carbon_offset = round(total * 0.0067, 1)
    area_restored = round(trees_funded / 500, 2)  # ~500 trees per km²
...
def profile():
    ...
    if request.method == 'POST' and db:
        updates = {k: v.strip() for k, v in request.form.items() if v.strip() and k != 'csrf_token'}
        if updates:
            try:
...
def discover():
    ...
    if db:
        try:
            ...
            logger.error(f"NGO discovery fetch: {e}")
